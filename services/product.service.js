...
} = require("../utility/getRankedProductsByStore");
const { fetchStoreGallery } = require("../utility/getStoreGallery");
const { getStoreReviews } = require("../utility/getStoreReviews");
const {
  getVariationAttributes,
} = require("../utility/product/getProductAttributes");
const {
  getProductTotalOrder,
} = require("../utility/product/getProductTotalOrder");
...
const {
  productsSearch,
  product_search_by_collections,
} = require("../utility/product/productSearch");
...
module.exports.barcode_search = async (req) => {
  try {
    ...
    console.error(
      "Error forwarding barcode to Flask server:",
      error.response?.data
    );
    return {
      success: false,
...
module.exports.fetch_multiple_products = async (req) => {
  ...
  try {
    ...
    const response = await axios.post(
      "https://api-bloomzon-com.onrender.com/products",
      payload
    );
...
module.exports.search_store_products = async (req) => {
  ...
  try {
    ...
    const { products, level_name } = await searchStore(
      { store_id, level, level_id, search },
      pool,
      ["name"]
    );
  ...
  try {
    ...
    if (activeCollection) {
      filtered = filtered.filter(
        (p) => p.collection.id === parseInt(activeCollection)
      );
    }
