...
module.exports.create_account = async (req: Request) => {
  const { firstName, lastName, email, phone, city, password, picture } =
    req.body;
  ...
  try {
    ...
    // Insert  auth record
    const [{ insertId: userAuthInsert }] = await connection.query(
      `INSERT INTO auth_table (user_id, email, password)
             VALUES (?, ?, ?)`,
      [insertId, trimmedEmail, hashedPassword]
    );
...
module.exports.login_User = async (req: Request) => {
  ...
  try {
    const [[access]] = await pool.query(
      "SELECT id, user_id, email, password, status FROM auth_table WHERE email = ?",
      [userEmail]
    );
    ...
    if (access && access.id) {
      ...
      if (passwordMatch) {
        if (access.status === 1) {
          ...
          if (userData && userData.id) {
            ...
            return {
              success: false,
              error:
                "Inactive account. Please contact the support for more details",
            };
          }
          return {
            success: false,
            error:
              "Unable to resolve account. Please contact the support or try again later",
          };
        }
        ...
        return {
          success: false,
          error:
            "This account is currently is inactive. Please contact support for more details",
        };
      }
