...
exports.addToCart = async (req, res) => {
    ...
    if (!user_id || !store_id || !product_id || !variation_id || !sku || !attribute_key || !quantity || !price || !weight) {
        return {
            success: false,
        ...
        if (existingItem.length > 0) {
            ...
            const [newItem] = await conn.query(
                `INSERT INTO cart_items 
                 ...
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                [cart_id, store_id, product_id, variation_id, sku.trim(), attribute_key.trim(), quantity, price, weight]
            );

            const cart_item_id = newItem.insertId;

            // 5. Insert attribute rows
            const attributesToInsert = Object.entries(selectedAttributes).map(([name, value]) => [
                cart_item_id,
                name.trim(),
                value.trim()
            ]);
