...
router.post("/fetch-single-store", async (req, res) => {
  try {
    ...
    if (result && result.success) {
      ...
      const errorMessage =
        result.error || "Error establishing connection to the server";
      res.status(400).json({ success: false, error: errorMessage });
    }
  } catch (error) {
...
router.post("/tips/add", async (req, res) => {
  try {
    const result = await service.add_tips(req);
...
router.post("/tips/products", async (req, res) => {
  try {
    const result = await service.update_tip_products(req);
    ...
    console.error("Error updating tips products:", error);
    res.status(500).json({ success: false, error: "Internal Server Error" });
  }
});

router.get("/tips/:store_id", async (req, res) => {
  try {
    const result = await service.fetch_store_tips(req);
...
router.get("/trends/:store_id", async (req, res) => {
  try {
    const result = await service.fetch_store_trends(req);
...
router.get("/videos/:store_id", async (req, res) => {
  try {
    const result = await service.fetch_store_video_interactions(req);
...
router.post("/follow-like", async (req, res) => {
  try {
