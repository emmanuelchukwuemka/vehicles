      ...
      db.query(SQL, (error, result) => {
        ...
        if (result.length === 0) {
          ...
          db.query(SQL, [userID, shopID, date, date], (error, data) => {
            ...
            db.query(
              SQL,
              [userID, shopID, message, messageType, userID, date],
              (error, data) => {
                if (error) {
                  console.error(error);
                  return;
          ...
          db.query(SQL, (error, result) => {
            ...
            const SQL = `UPDATE conversations SET count = ${
              count + 1
            } WHERE sender_id = ${userID} AND receiver_id =${shopID} OR sender_id = ${shopID} AND receiver_id =${userID} `;
