  ...
  db.query(SQL,(err,result)=>{
      if(err)
      {
       ...
       if(result.length!=0){
          ...
          db.query(SQL,(err,data)=>{
            if(err)
            {
              ...
              if(data.length!=0){
                ...
                db.query(SQL,[id],(err,data)=>{if(err) return res.status(500).json({message: "Error Querying database"});})
          ...
          db.query(SQL,(err,data)=>{
            if(err)
            {
              ...
              if(data.length!=0){
                ...
                db.query(SQL,(err,data)=>{if(err) return res.status(500).json({message: "Error Querying database"});})
        ...
        sendEmail(data)
         return res.status(201).json({message: "Account usertype updated successfully",status:201});
            
       }else{
         
          return res.status(401).json({message: "Failed to update account type",status:401});
