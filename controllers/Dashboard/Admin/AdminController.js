...
const createAdmins = asyncHandler( async (req,res)=>{
    ...
    db.query(SQL,(err,result)=>{
        ...
        if(result.length===0)
         {
           return res.status(401).json({message: "An account with this does not existed please signup",status:404});
              
         }else if(validator.isEmail(email)===false){

          return res.status(401).json({message: "Email address is invalid",status:404});
          ...
          db.query(SQL,[staff_id,type,enabled,creatorId,currentDate()],(err,result)=>
          {
            ...
            sendEmail(data)
            return res.status(201).json({message: "Account Created Successfully",status:1});
              }
          }); 
         }}
        }) 
