...
const getAds = asyncHandler(async (req, res, next) => {
    ...
    if (searchType === "home") {
        ...
        else if ((bedRoomFilter !== "undefined") &
        ...
        db.query(SQL, (error, result) => {
            if (error) {
                ...
                const data = result.filter((data) => (data.deleted === 0) & (data.drafted === 0));
...
const createmessage = asyncHandler(async (req, res) => {
    ...
    db.query(SQL, (error, result) => {
        ...
        if (result.length === 0) {
            ...
            db.query(SQL, [userID, shopID, propertyID, message, date, date], (error, data) => {
                if (error) {
                    console.error(error);
                    return;
                }
                const SQL = `INSERT INTO messagesRealEstate (sender_id,receiver_id,message,creator_id,date,property_id)VALUES(?,?,?,?,?,?)`;
                db.query(SQL, [userID, shopID, message, userID, date, propertyID], (error, data) => {
                    if (error) {
                        console.error(error);
                        return;
                    }
            ...
            db.query(SQL, (error, result) => {
                ...
                const SQL = `UPDATE conversationsRealEstate SET  message = \'${message}\', count = ${count + 1} WHERE id = ${id}`;
                db.query(SQL, (error, result) => {
                    if (error) {
                        console.error(error);
                        return;
