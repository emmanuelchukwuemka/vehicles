...
exports.addToCart = async (req, res) => {
    const { user_id, store_id, product_id, variation_id, sku, key: attribute_key, quantity, price, weight, selectedAttributes = {}, } = req.body;
    if (!user_id ||
        !store_id ||
        !product_id ||
        !variation_id ||
    ...
    try {
        ...
        let [existingCart] = await conn.query("SELECT id FROM carts_table WHERE user_id = ? LIMIT 1", [user_id]);
        let cart_id;
        if (existingCart.length > 0) {
            ...
            try {
                const [cartResult] = await conn.query("INSERT INTO carts_table (user_id) VALUES (?)", [user_id]);
                cart_id = cartResult.insertId;
            }
            catch (insertErr) {
                // Fallback in case of race condition duplicate insert
                if (insertErr.code === "ER_DUP_ENTRY") {
                    const [retryCart] = await conn.query("SELECT id FROM carts_table WHERE user_id = ? LIMIT 1", [user_id]);
        ...
        const [existingItem] = await conn.query(`SELECT id, quantity FROM cart_items 
             WHERE cart_id = ? AND sku = ? AND attribute_key = ?`, [cart_id, sku.trim(), attribute_key.trim()]);
        if (existingItem.length > 0) {
            // 3. Update quantity
            await conn.query(`UPDATE cart_items 
                 SET quantity = quantity + ?, price = ? 
                 WHERE id = ?`, [quantity, price, existingItem[0].id]);
        }
        else {
            // 4. Insert new item
            const [newItem] = await conn.query(`INSERT INTO cart_items 
            ...
            const attributesToInsert = Object.entries(selectedAttributes).map(([name, value]) => [cart_item_id, name.trim(), value.trim()]);
            if (attributesToInsert.length > 0) {
                await conn.query(`INSERT INTO cart_item_attributes (cart_item_id, name, value) VALUES ?`, [attributesToInsert]);
            }
        }
        await conn.commit();
        return {
...
exports.updateCartItem = async (req) => {
    ...
    try {
        ...
        // Step 1: Get cart id
        const [cart] = await connection.query("SELECT id FROM carts_table WHERE user_id = ? LIMIT 1", [user_id]);
        if (cart.length === 0) {
            return { success: false, error: "Cart not found for user" };
        }
        const cartId = cart[0].id;
        // Step 2: Find the item
        const [item] = await connection.query("SELECT id FROM cart_items WHERE cart_id = ? AND sku = ? AND attribute_key = ? LIMIT 1", [cartId, sku.trim(), attribute_key.trim()]);
        if (item.length === 0) {
            return { success: false, error: "Cart item not found" };
        }
        // Step 3: Update quantity
        const [qtyUpdate] = await connection.query("UPDATE cart_items SET quantity = ? WHERE id = ?", [quantity, item[0].id]);
...
exports.removeCartItem = async (req) => {
    ...
    try {
        ...
        const [cartRows] = await connection.query("SELECT id FROM carts_table WHERE user_id = ?", [jwtData.user_id]);
...
exports.getCart = async (req) => {
    ...
    try {
        // Get the cart ID
        const [cart] = await conn.query("SELECT id FROM carts_table WHERE user_id = ?", [user_id]);
        if (cart.length === 0) {
        ...
        const [scopeRows] = await conn.query(`SELECT store_id, scope FROM stores_scope WHERE store_id IN (?)`, [storeIds]);
        // STEP 3: Group scopes by store_id
        const scopeMap = {};
        scopeRows.forEach(({ store_id, scope }) => {
            if (!scopeMap[store_id])
                scopeMap[store_id] = [];
        ...
        // Fetch MOQ data
        const [moqData] = await conn.query(`SELECT product_id, min_qty, ppu FROM product_moq WHERE product_id IN (?) ORDER BY min_qty DESC`, [productIds]);
        // Group MOQ by product_id
        const moqMap = {};
        moqData.forEach((row) => {
            if (!moqMap[row.product_id])
                moqMap[row.product_id] = [];
        ...
        const [attributeRows] = await conn.query(`SELECT cart_item_id AS item_id, name, value FROM cart_item_attributes WHERE cart_item_id IN (${placeholders})`, cartItemIds);
        // Group attributes by item_id
        const attributeMap = {};
        attributeRows.forEach((attr) => {
            if (!attributeMap[attr.item_id])
                attributeMap[attr.item_id] = {};
