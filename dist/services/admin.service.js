...
module.exports.change_user_scope = async (req) => {
    ...
    try {
        const [rows] = await pool.query("SELECT id, status FROM stores_table WHERE id = ? LIMIT 1", [store_id]);
        if (rows.length > 0) {
            ...
            const [result] = await pool.query("INSERT IGNORE INTO stores_scope (store_id, scope) VALUES (?, ?)", [storeData.id, scope] // e.g. 3, 'seller'
            );
            ...
            return {
                success: true,
                data: scopes, // the single user object
            };
        }
        return {
            success: false,
...
module.exports.create_ability = async (req) => {
    ...
    try {
        ...
        const [existingAbilities] = await pool.query("SELECT id FROM capabilities_table WHERE name = ? LIMIT 1", [name]);
        ...
        const [{ insertId }] = await pool.query("INSERT INTO capabilities_table (name, description, created_at, updated_at) VALUES (?, ?, NOW(), NOW())", [newName, description]);
        return {
            success: true,
...
module.exports.create_filter = async (req) => {
    ...
    try {
        // Check if ability already exists
        const [existingFilter] = await pool.query("SELECT id FROM filters_table WHERE name = ? LIMIT 1", [name]);
        ...
        // Insert the new ability
        const [{ insertId }] = await pool.query("INSERT INTO filters_table (name, description, status, created_at, updated_at) VALUES (?, ?, 1, NOW(), NOW())", [filterName, filterDesc]);
        return {
            success: true,
...
module.exports.assign_filter_to_product = async (req) => {
    ...
    try {
        ...
        const insertValues = validFilterIds.map((filter_id) => [
            product_id,
            filter_id,
        ]);
...
module.exports.create_shipping_method = async (req) => {
    ...
    try {
        ...
        const [existingMethod] = await pool.query("SELECT id FROM shipping_methods WHERE name = ? LIMIT 1", [trim_name]);
        ...
        const [{ insertId }] = await pool.query("INSERT INTO shipping_methods (name, description, icon, status, created_at) VALUES (?, ?, ?, 1, NOW())", [trim_name, trim_desc, trim_icon]);
        return {
            success: true,
...
module.exports.add_shipping_provider = async (req) => {
    const { method_id, name, duration_from, duration_to, price, class: shipping_class, notice, is_guaranteed, offered_by, } = req.body;
    ...
    try {
        ...
        const [methodExists] = await connection.query("SELECT id FROM shipping_methods WHERE id = ? LIMIT 1", [method_id]);
        ...
        const [existingProvider] = await connection.query("SELECT id FROM shipping_providers WHERE method_id = ? AND name = ? LIMIT 1", [method_id, trim_Name]);
        ...
        // Insert new provider
        const [{ insertId }] = await connection.query(`INSERT INTO shipping_providers 
...
module.exports.createStoreGalleryItem = async (req, res) => {
    ...
    try {
        ...
        if (storeRows.length === 0) {
            ...
            return {
                success: false,
                error: "Store does not exist or is not active.",
            };
        }
        // Insert into store_gallery
        const [result] = await connection.query(`INSERT INTO store_gallery (store_id, url, type, position, title, description) VALUES (?, ?, ?, ?, ?, ?)`, [
            store_id,
            url.trim(),
            type.trim(),
            position.trim(),
...
module.exports.createPaymentGateway = async (req, res) => {
    ...
    try {
        await connection.beginTransaction();
        const [existing] = await connection.query(`SELECT id FROM payment_gateways WHERE provider = ? LIMIT 1`, [provider.trim()]);
        ...
        const [result] = await connection.query(`INSERT INTO payment_gateways (name, logo, provider, is_active, sandbox_mode, config)
             VALUES (?, ?, ?, ?, ?, ?)`, [
            name.trim(),
            logo,
            provider.trim(),
...
module.exports.createLayout = async (req) => {
    ...
    if (!name || !description || !priority) {
        return {
            success: false,
            error: "All fields (name, description, priority) are required",
        };
    }
...
module.exports.createAttribute = async (req) => {
    ...
    try {
        await connection.beginTransaction();
        const [existing] = await connection.query("SELECT id FROM attributes_table WHERE name = ?", [name.trim()]);
        if (existing.length > 0) {
            connection.release();
            return {
                success: false,
        ...
        const [result] = await connection.query(`INSERT INTO attributes_table (name, label, layout_id) VALUES (?, ?, ?)`, [name.trim(), label, layout_id]);
        await connection.commit();
        return {
            success: true,
    ...
    try {
        ...
        for (const row of rows) {
            ...
            if (Object.keys(updates).length > 0) {
                ...
                values.push(row.id);
                await connection.query(`UPDATE \`${table}\` SET ${setClause} WHERE id = ?`, values);
            }
        }
        // await connection.rollback();
        // return {
