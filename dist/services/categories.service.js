...
module.exports.add_mainCategory = async (req) => {
    ...
    try {
        ...
        const [[sectionData]] = await pool.query(`SELECT * FROM maincategory WHERE _name = ?`, [enc_name]);
        ...
        const [{ affectedRows }] = await pool.query(`INSERT INTO maincategory (_name, _image, _status, _date) VALUES (?, ?, ?, NOW())`, [enc_name, enc_image, enc_status]);
        if (affectedRows > 0) {
            return {
                success: true,
...
module.exports.add_category = async (req) => {
    ...
    try {
        ...
        const [section] = await pool.query(`SELECT id FROM maincategory WHERE id = ?`, [main_id]);
        if (section.length === 0) {
            return {
                success: false,
        ...
        const [existingCategory] = await pool.query(`SELECT id FROM category WHERE _name = ? AND _maincategory = ?`, [cat_name, main_id]);
        ...
        const [{ affectedRows }] = await pool.query(`INSERT INTO category (_name, _maincategory, _status, _date) VALUES (?, ?, ?, NOW())`, [cat_name, main_id, cat_status]);
        if (affectedRows > 0) {
            return {
                success: true,
...
module.exports.add_subCategory = async (req) => {
    ...
    try {
        ...
        const [categoryRows] = await pool.query(`SELECT id FROM category WHERE id = ?`, [categoryId]);
        ...
        const [existingSubCategory] = await pool.query(`SELECT id FROM subcategory WHERE _name = ? AND _category = ?`, [enc_name, categoryId]);
        ...
        const [{ affectedRows }] = await pool.query(`INSERT INTO subcategory (_name, _category, _image, _status, _date) VALUES (?, ?, ?, ?, NOW())`, [enc_name, categoryId, enc_image, enc_status]);
        if (affectedRows > 0) {
            return {
                success: true,
...
module.exports.multi_insertion = async (req) => {
    ...
    try {
        ...
        const [categoryRows] = await pool.query(`SELECT id FROM category WHERE id = ?`, [id]);
        if (categoryRows.length === 0) {
            return {
                success: false,
                error: "Category with the provided ID does not exist",
        ...
        for (const item of data) {
            ...
            const [existingSubCategory] = await pool.query(`SELECT id FROM subcategory WHERE _name = ? AND _category = ?`, [enc_name, id]);
            if (existingSubCategory.length > 0) {
                skippedCount++;
                continue; // Skip if sub-category already exists
            }
            // Insert new sub-category
            const [{ affectedRows }] = await pool.query(`INSERT INTO subcategory (_name, _category, _image, _status, _date) VALUES (?, ?, ?, ?, NOW())`, [enc_name, id, enc_image, enc_status]);
            if (affectedRows > 0) {
                insertedCount++;
            }
            else {
