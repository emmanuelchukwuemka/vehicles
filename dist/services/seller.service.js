...
const { getProductIdsByStore, } = require("../utility/product/getProductIdsByStore");
const { enrichProductsWithVariations, } = require("../utility/product/enrichProductsWithVariations");
const { getVideoMediaByStore, } = require("../utility/product/getVideoMediaByStore");
const { getRecordedLiveVideosByStore, } = require("../utility/product/getRecordedLiveVideosByStore");
const { normalizeVideos } = require("../utility/media/normalizeVideos");
const { getStoreTrendingProducts, } = require("../utility/store/getStoreTrendingProducts");
const { getVideoInteractionMap, } = require("../utility/media/getVideoInteractionMap");
...
module.exports.create_store = async (req) => {
    ...
    try {
        ...
        const placeholders = Object.keys(mergedData)
            ...
            .join(", ");
        const values = Object.values(mergedData);
        const [{ insertId }] = await connection.query(`INSERT INTO stores_table (${columns}, created_at, updated_at) VALUES (${placeholders}, NOW(), NOW())`, values);
        if (insertId) {
            ...
            if (capabilities.length > 0) {
                ...
                await connection.query(`INSERT INTO store_capabilities (store_id, capability_id) VALUES ?`, [capabilities_IDs]);
            }
            await connection.commit();
            return {
                success: true,
...
module.exports.createBaseProduct = async (req) => {
    ...
    try {
        ...
        const [{ insertId: productId }] = await connection.query(`INSERT INTO products_table 
        ...
        if (sample && sample.ppu != null && sample.min_qty != null) {
            ...
            await connection.query(`INSERT INTO product_sample (store_id, product_id, ppu, min_qty, created_at, updated_at) VALUES (?, ?, ?, ?, NOW(), NOW()) ON DUPLICATE KEY UPDATE ppu = VALUES(ppu), min_qty = VALUES(min_qty),  updated_at = NOW()`, [store_id, productId, sample.ppu, sample.min_qty]);
        }
        // 🔹 Insert MOQ (if available)
        if (Array.isArray(moq)) {
            for (const { min_qty, ppu } of moq) {
                await connection.query(`INSERT INTO product_moq (product_id, min_qty, ppu, created_at, updated_at)
                             VALUES (?, ?, ?, NOW(), NOW())`, [productId, min_qty, ppu]);
            }
        }
        // 🔹 Insert Specifications (if available)
        if (Array.isArray(specifications)) {
            for (const { name: specName, value: specValue } of specifications) {
                await connection.query(`INSERT INTO product_specifications (product_id, name, value, created_at)
                     VALUES (?, ?, ?, NOW())`, [productId, specName, specValue]);
            }
        }
        await connection.commit();
        return {
...
module.exports.createProductVariation = async (req) => {
    ...
    try {
        ...
        const [[productData]] = await connection.query(`SELECT id, product_code, subcategory_id, name FROM products_table WHERE id = ? AND product_code = ? LIMIT 1`, [product_id, product_code]);
        if (!productData) {
            //await connection.rollback();
            return {
                success: false,
        ...
        const [{ insertId: variationId }] = await connection.query(`INSERT INTO variations_table (product_id, sku, status, created_at, updated_at)
            VALUES (?, ?, 1, NOW(), NOW())`, [product_id, variationSku]);
        if (!variationId) {
            await connection.rollback();
            return {
                success: false,
        ...
        for (const attribute of variation.attributes || []) {
            ...
            const [attributeResult] = await connection.query(`INSERT INTO variation_attributes (variation_id, attribute_id, label, value, price, stock, weight, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`, [variationId, attribute_id, label, value, price, stock, weight]);
            const insertedAttributeId = attributeResult.insertId;
            // Insert image into media_table if present
            if (image) {
                await connection.query(`INSERT INTO media_table (product_id, variation_id, attribute_id, url, type, created_at)
             VALUES (?, ?, ?, ?, ?, NOW())`, [product_id, variationId, insertedAttributeId, image, "image"]);
            }
        }
        // Insert gallery files (if available)
        for (const { url, type } of variation.media || []) {
            await connection.query(`INSERT INTO media_table (product_id, variation_id, url, type, created_at) VALUES (?, ?, ?, ?, NOW())`, [product_id, variationId, url, type]);
        }
        await connection.commit();
        return {
            success: true,
...
module.exports.save_live = async (req) => {
    ...
    const { store_id, product_id, title, description = "", stream_url, thumbnail_url = null, } = req.body;
    try {
        // Begin transaction
        await conn.beginTransaction();
        // Validate store
        const [storeRows] = await conn.query(`SELECT id FROM stores_table WHERE id = ? AND status = 1 LIMIT 1`, [store_id]);
        ...
        const [productRows] = await conn.query(`SELECT id, store_id FROM products_table WHERE id = ? AND status = 1 LIMIT 1`, [product_id, store_id]);
        ...
        // Insert live session
        const [result] = await conn.query(`INSERT INTO live_table 
...
module.exports.fetch_store_products = async (req) => {
    ...
    try {
        const [products] = await pool.query("SELECT * FROM products_table WHERE store_id = ? ORDER BY created_at DESC", [store_id]);
...
module.exports.fetch_store_collections = async (req) => {
    ...
    try {
        ...
        const [collections] = await pool.query(`SELECT DISTINCT c.id, c.name 
             FROM collections_table c
             JOIN products_table p ON c.id = p.collection_id
             WHERE p.store_id = ? AND c.status = 1`, [store_id]);
        return {
            success: true,
            data: collections,
        };
...
module.exports.fetch_product = async (req) => {
    ...
    try {
        ...
        const [productRows] = await pool.query(`SELECT * FROM products_table WHERE id = ?`, [product_id]);
        if (productRows.length === 0) {
            return {
                success: false,
        ...
        const product = productRows[0];
        // Fetch variations
        const [variations] = await pool.query(`SELECT * FROM variations_table WHERE product_id = ?`, [product_id]);
        for (const variation of variations) {
            // Fetch attributes for each variation
            const [attributes] = await pool.query(`SELECT name, value FROM variation_attributes WHERE variation_id = ?`, [variation.id]);
            // Fetch media for each variation
            const [media] = await pool.query(`SELECT url, type FROM media_table WHERE variation_id = ?`, [variation.id]);
            variation.attributes = attributes;
            variation.media = media;
        }
        // Fetch MOQ
        const [moq] = await pool.query(`SELECT min_qty, ppu FROM product_moq WHERE product_id = ?`, [product_id]);
        return {
            success: true,
            data: {
                ...product,
...
module.exports.add_tips = async (req) => {
    ...
    if (isNaN(store_id) ||
        ...
        return {
            success: false,
            error: "Invalid input. Please provide valid store_id, products, media_type, media_url, and description.",
        };
    }
    try {
        // 1. Validate store exists
        const [storeRows] = await pool.query(`SELECT id FROM stores_table WHERE id = ?`, [store_id]);
        if (storeRows.length === 0) {
            return {
                success: false,
        ...
        const [productRows] = await pool.query(`SELECT id FROM products_table WHERE id IN (?) AND store_id = ?`, [products, store_id]);
        const foundProductIds = productRows.map((row) => row.id);
        const missingProductIds = products.filter((id) => !foundProductIds.includes(id));
        ...
        // 3. Insert the tip
        const [result] = await pool.query(`INSERT INTO store_tips (store_id, media_type, media_url, description, products, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, NOW(), NOW())`, [store_id, media_type, media_url, description, JSON.stringify(products)]);
        return {
            success: true,
            data: {
                id: result.insertId,
...
module.exports.update_tip_products = async (req) => {
    ...
    try {
        ...
        const [tipRows] = await pool.query(`SELECT store_id, products FROM store_tips WHERE id = ?`, [tip_id]);
        if (tipRows.length === 0) {
            return {
                success: false,
                error: "Tip not found",
        ...
        const [productRows] = await pool.query(`SELECT id FROM products_table WHERE id = ? AND store_id = ?`, [product_id, store_id]);
        if (productRows.length === 0) {
            return {
                success: false,
        ...
        // 5. Update the row
        await pool.query(`UPDATE store_tips SET products = ?, updated_at = NOW() WHERE id = ?`, [JSON.stringify(currentProducts), tip_id]);
        return {
            success: true,
            data: {
...
module.exports.fetch_store_tips = async (req) => {
    ...
    try {
        ...
        const [tips] = await pool.query(`SELECT id, media_url, description, products FROM store_tips WHERE store_id = ? ORDER BY created_at DESC`, [store_id]);
        if (!tips.length) {
            return {
                success: true,
                data: [],
...
module.exports.fetch_store_video_interactions = async (req) => {
    ...
    try {
        ...
        const [productRows] = await pool.query(`SELECT id FROM products_table WHERE store_id = ?`, [store_id]);
...
module.exports.follow_and_like_store = async (req) => {
    ...
    try {
        ...
        const [storeRows] = await conn.query("SELECT id FROM stores_table WHERE id = ? AND status = 1", [store_id]);
        if (storeRows.length === 0) {
            await conn.rollback();
            return { success: false, error: "Store not found or inactive" };
        }
        ...
        const [existing] = await conn.query("SELECT id FROM interactions WHERE user_id = ? AND target_type = ? AND target_id = ? AND action = ?", [user_id, target_type, store_id, action]);
        if (existing.length > 0) {
            // If exists, remove interaction (unlike/unfollow)
            await conn.query("DELETE FROM interactions WHERE user_id = ? AND target_type = ? AND target_id = ? AND action = ?", [user_id, target_type, store_id, action]);
            await conn.commit();
            return { success: true, data: 0 }; // 0 means removed
        }
        else {
            // If not exists, add interaction (like/follow)
            await conn.query("INSERT INTO interactions (user_id, target_type, target_id, action) VALUES (?, ?, ?, ?)", [user_id, target_type, store_id, action]);
            await conn.commit();
