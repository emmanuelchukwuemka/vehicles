...
const getVendorByUserId_1 = require("../utility/user/getVendorByUserId");
module.exports.create_account = async (req) => {
    ...
    try {
        ...
        // Insert new vendor
        const [{ insertId }] = await connection.query(`INSERT INTO users_table (${columns}, created_at, updated_at)
             VALUES (${placeholders}, NOW(), NOW())`, values);
        if (!insertId) {
            await connection.rollback();
...
module.exports.login_User = async (req) => {
    ...
    try {
        ...
        if (userData && userData.id) {
            ...
            if (passwordMatch) {
                const vendor = await (0, getVendorByUserId_1.checkVendorByUserId)({
                    pool,
                    user_id: userData.id,
                });
                if (vendor) {
...
module.exports.create_vendor_account = async (req) => {
    console.log("testing");
    ...
    try {
        connection = await pool.getConnection();
        await connection.beginTransaction();
        const [existingUser] = await connection.query(`SELECT id FROM users_table WHERE id = ? LIMIT 1`, [user_id]);
        if (existingUser.length === 0) {
            await connection.rollback();
        ...
        const values = Object.values(vendorData);
        // Insert new vendor
        const [{ insertId }] = await connection.query(`INSERT INTO vendors_table (${columns})
...
module.exports.add_address = async (req) => {
    const { user_id, country, state, name, contact, postal_code, address, city, label, is_default = 0, } = req.body;
    if (!user_id ||
        !country ||
        !state ||
        !name ||
    ...
    try {
        connection = await pool.getConnection();
        await connection.beginTransaction();
        const [existing] = await connection.query(`SELECT id FROM user_addresses WHERE user_id = ? AND address = ? AND postal_code = ? AND phone = ?`, [user_id, address, postal_code, contact]);
        ...
        if (is_default == 1) {
            await connection.query(`UPDATE user_addresses SET is_default = 0 WHERE user_id = ?`, [user_id]);
        }
        const [result] = await connection.query(`INSERT INTO user_addresses 
...
module.exports.get_user_addresses = async (req) => {
    ...
    try {
        const [addresses] = await pool.query(`SELECT id, name, phone, address, city, state, postal_code, country, is_default, created_at FROM user_addresses
             WHERE user_id = ?
...
module.exports.get_product_reviews = async (req) => {
    ...
    try {
        ...
        const [productCheck] = await pool.query(`SELECT id FROM products_table WHERE id = ? AND status = 1 LIMIT 1`, [product_id]);
        ...
        const [reviewStats] = await pool.query(`SELECT 
                COUNT(id) AS total_reviews, 
                IFNULL(AVG(rating), 0) AS average_rating
             FROM product_reviews 
             WHERE product_id = ? AND status = 1`, [product_id]);
        ...
        const [userReviews] = await pool.query(`SELECT 
                pr.id,
                pr.user_id,
             ...
             ORDER BY pr.created_at DESC`, [product_id]);
        return {
            success: true,
            data: {
...
module.exports.give_product_review = async (req) => {
    ...
    try {
        ...
        const [existingReview] = await pool.query(`SELECT id FROM product_reviews WHERE product_id = ? AND user_id = ? LIMIT 1`, [product_id, user_id]);
        ...
        await pool.query(`INSERT INTO product_reviews (product_id, user_id, rating, review_text, user_ip) 
             VALUES (?, ?, ?, ?, ?)`, [product_id, user_id, rating, review_text, user_ip]);
        return {
            success: true,
...
module.exports.give_store_review = async (req) => {
    ...
    try {
        ...
        await pool.query(`INSERT INTO store_reviews (store_id, user_id, rating, product_quality, supplier_service, on_time_shipment, review_text, user_ip) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [
            store_id,
            user_id,
            rating,
...
module.exports.get_shipping_providers = async () => {
    try {
        const [rows] = await pool.query("SELECT * FROM shipping_providers WHERE status = 1");
        const classMap = {
            0: "Basic",
            1: "Premium",
            2: "Elite",
...
exports.getPaymentGateways = async () => {
    try {
        const [gateways] = await pool.query(`SELECT id, name, logo, provider, is_active FROM payment_gateways ORDER BY name ASC`);
...
exports.placeNewOrder = async (req) => {
    ...
    try {
        ...
        const [orderResult] = await connection.query(`INSERT INTO orders_table 
        ...
        for (const item of orderItems) {
            ...
            const [orderItemResult] = await connection.query(`INSERT INTO order_items 
            ...
            if (order_item_id) {
                ...
                for (const [key, value] of Object.entries(item?.selectedAttributes)) {
                    await connection.query(`INSERT INTO order_item_attributes (order_item_id, attribute_name, attribute_value) VALUES (?, ?, ?)`, [order_item_id, key, value]);
                }
            }
...
exports.getUserCards = async (req) => {
    const { user_id } = req.body;
    if (!user_id) {
        return {
            success: false,
...
exports.removeUserCard = async (req) => {
    ...
    try {
        const [result] = await pool.query(`DELETE FROM user_payment_methods WHERE id = ? AND user_id = ?`, [card_id, user_id]);
        ...
        const [cards] = await pool.query(`SELECT * FROM user_payment_methods WHERE user_id = ? ORDER BY is_default DESC, created_at DESC`, [user_id]);
        return {
            success: true,
            data: cards,
        };
