...
module.exports.getStoreReviews = async (pool, store_id) => {
    ...
    try {
        ...
        const [reviewStats] = await pool.query(`SELECT
             ...
             FROM store_reviews
             WHERE store_id = ? AND status = 1`, [store_id]);
        ...
        const [categoryRatings] = await pool.query(`SELECT 
        ...
        const product_quality = categoryData.avg_product_quality !== null
            ? parseFloat(categoryData.avg_product_quality).toFixed(1)
            : null;
        const supplier_service = categoryData.avg_supplier_service !== null
            ? parseFloat(categoryData.avg_supplier_service).toFixed(1)
            : null;
        const on_time_shipment = categoryData.avg_on_time_shipment !== null
        ...
        const [userReviews] = await pool.query(`SELECT 
             ...
             ORDER BY sr.created_at DESC LIMIT 4`, [store_id]);
        return {
            success: true,
            data: {
