...
module.exports.getProductSubcategory = async (productIds) => {
    try {
        ...
        const [productRows] = await pool.query(`SELECT id as product_id, subcategory_id FROM products_table WHERE id IN (?)`, [ids]);
        const subcategoryIds = [...new Set(productRows.map(p => p.subcategory_id).filter(Boolean))];
        if (!subcategoryIds.length)
            return [];
        // Step 2: Get subcategory details
        const [subcategoryRows] = await pool.query(`SELECT id, _name AS name, _image AS image FROM subcategory WHERE id IN (?)`, [subcategoryIds]);
