...
module.exports.categorizeReview = (req) => {
    ...
    if (!review_text)
        return { product_quality: rating, supplier_service: rating, on_time_shipment: rating };
...
module.exports.confirmCardPayment = async (payment) => {
    ...
    try {
        const response = await axios.post("https://bloomzonapi-6idf.onrender.com/paystack/payment_pin_validation", payment);
...
module.exports.saveCardPaymentMethod = async (connection, cardInfo) => {
    ...
    const { user_id, method, brand, last4, expiry_month, expiry_year, token, paymentGateway_id, isDefaultCard } = cardInfo;
    try {
        if (!!isDefaultCard) {
            // Unset other default cards for the user
            await connection.query(`UPDATE user_payment_methods SET is_default = 0 WHERE user_id = ?`, [user_id]);
        }
        await connection.query(`INSERT INTO user_payment_methods 
...
module.exports.saveCustomization = async (connection, customData) => {
    const { product_id, store_id, user_id, variation_id, sku, attr_key, source, customizations } = customData;
    try {
        for (const key in customizations) {
            const { type: customization_type, customization_id = null, value, settings } = customizations[key];
            const query = `
