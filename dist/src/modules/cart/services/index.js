...
const associations_1 = require("../../product/models/associations");
const store_models_1 = require("../../store/models/store.models");
const cartAssociations_1 = require("../models/cartAssociations");
...
class CartService {
    ...
    static async getOrCreateCart(params) {
        ...
        let cart = null;
        if (cartId) {
            cart = await cartAssociations_1.Cart.findByPk(cartId, { transaction });
        }
        else {
            cart = await cartAssociations_1.Cart.findOne({ where: { user_id: userId }, transaction });
        }
        if (!cart && createIfMissing) {
            cart = await cartAssociations_1.Cart.create({ user_id: userId }, { transaction });
        }
        return cart;
    }
    // Add Item to Cart
    static async addItemToCart(input, opts) {
        if (!input.user_id) {
        ...
        try {
            // 1) Validate user
            const user = await user_models_1.default.findByPk(input.user_id, { transaction });
            if (!user) {
            ...
            // 3) Validate product
            const product = await associations_1.Product.findByPk(input.product_id, { transaction });
            if (!product) {
                await transaction.rollback();
                return { success: false, message: "Product not found" };
            }
            // 4) Validate store (belongs to product)
            const store = await store_models_1.Store.findByPk(product.store_id, { transaction });
            if (!store) {
                await transaction.rollback();
            ...
            let unitRow = null;
            if (input.unit_id) {
                unitRow = await associations_1.ProductUnit.findByPk(input.unit_id, { transaction });
                if (!unitRow) {
                    await transaction.rollback();
            ...
            const existing = await cartAssociations_1.CartItem.findOne({
                where: {
                    cart_id: cart.id,
            ...
            if (existing) {
                ...
                await cartAssociations_1.CartItem.create({
                    cart_id: cart.id,
                    store_id: store.id,
    ...
    static async mergeGuestCartToUser(userId, guestItems) {
        const transaction = await sequelize_1.default.transaction();
        try {
            let cart = await cartAssociations_1.Cart.findOne({
            ...
            for (const item of guestItems) {
                const product = await associations_1.Product.findByPk(item.product_id, {
                    transaction,
                });
                if (!product)
                    continue;
                const store = await store_models_1.Store.findByPk(item.store_id, { transaction });
                if (!store)
                    continue;
                let unitRow = null;
                if (item.unit_id) {
                    unitRow = await associations_1.ProductUnit.findByPk(item.unit_id, { transaction });
                    if (!unitRow)
                        continue;
                    if (item.quantity > Number(unitRow.stock ?? 0))
                        continue;
                }
    ...
    //   Update cart item
    static async updateCartItem(input) {
        const transaction = await sequelize_1.default.transaction();
        try {
            // Find the cart item
            const item = await cartAssociations_1.CartItem.findByPk(input.cart_item_id, {
                transaction,
            ...
            const cart = await cartAssociations_1.Cart.findByPk(item.cart_id, { transaction });
            ...
            const product = await associations_1.Product.findByPk(item.product_id, { transaction });
            if (!product) {
                await transaction.rollback();
                return { success: false, message: "Product not found" };
            }
    ...
    //   Remove item
    static async removeCartItem(input) {
        const { user_id, cart_item_id } = input;
        const transaction = await sequelize_1.default.transaction();
        try {
            // 1) Find cart item
            const item = await cartAssociations_1.CartItem.findByPk(cart_item_id, { transaction });
            if (!item) {
                await transaction.rollback();
                return { success: false, message: "Cart item not found" };
            }
            ...
            const remainingItems = await cartAssociations_1.CartItem.count({
    ...
    static async clearCart(user_id) {
        ...
        try {
            ...
            const cart = await cartAssociations_1.Cart.findOne({ where: { user_id }, transaction });
            if (!cart) {
                await transaction.rollback();
                return { success: false, message: "Cart not found" };
            }
            // 2) Delete all items inside the cart
            await cartAssociations_1.CartItem.destroy({ where: { cart_id: cart.id }, transaction });
    ...
    static async getCart(user_id) {
        try {
            ...
            const cart = await cartAssociations_1.Cart.findOne({
                where: { user_id },
                attributes: { exclude: ["created_at", "updated_at"] },
                include: [
                    {
                        model: cartAssociations_1.CartItem,
                        as: "items",
                        attributes: { exclude: ["created_at", "updated_at"] },
                    },
                ],
