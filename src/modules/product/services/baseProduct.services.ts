...
export const createBaseProduct = async (input: BaseProductInput) => {
  ...
  try {
    ...
    // Step 6: Insert product-level medias
    ...
    // Step 7: Insert variations if provided
    if (Array.isArray(variations) && variations.length > 0) {
      ...
      const createdVariations = await Variation.bulkCreate(variationPayloads, {
        transaction,
      });
